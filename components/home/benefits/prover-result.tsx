    ...
    <section className='max-w-7xl mx-auto px-4 sm:px-6 md:px-10 lg:px-12 my-10 sm:my-16 md:my-20'>
      ...
      >
        ...
        <PageTitle
          title={title}
          align='center'
          titleClassName='text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white! text-center max-w-3xl mx-auto mb-8 sm:mb-12 md:mb-14'
        />
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6'>
          {stats.map((stat: {value: string, title: string, description: string}, index: number) => {
            ...
            return (
              <div
                key={index}
                className='flex flex-col gap-4 sm:gap-5 rounded-2xl p-5 sm:p-6 transition-transform duration-200 hover:-translate-y-1'
                style={{
                ...
                {/* Value */}
                <span
                  className='text-3xl sm:text-4xl md:text-[42px] font-serif font-bold leading-none'
                  style={{ color: config.valueColor }}>
                  {stat.value}
                </span>

                {/* Title */}
                <p className='font-sans text-white text-lg sm:text-xl font-semibold leading-snug'>
                  {stat.title}
                </p>
                ...
                <div className='mt-auto pt-3'>
                  <div
                    ...
                    style={{ background: config.accentColor, opacity: 0.8 }}
                  />
                </div>
              </div>
            );
