...
function CancelationEditModal({
  ...
  useEffect(() => {
    if (isOpen && rowData) {
      const content =
        pageData?.sections?.find((s: PageSection) => s.sectionKey === rowData.sectionKey)
          ?.content || {};
      reset(content);
    }
...
export default function CrmCancelationPage() {
  ...
  if (pageData) {
    ...
    rows.push({
      ...
      title: hero.title || 'We’d hate to see you go',
      subtitle: hero.subtitle || 'Before you cancel, would any of these options help?',
      type: 'hero',
    });
    ...
    rows.push({
      ...
      sectionKey: 'option1',
      section: 'Option 1',
      title: option1.text || 'Deliver less often',
      subtitle: '-',
      type: 'option',
    });
    ...
    rows.push({
      ...
      sectionKey: 'option2',
      section: 'Option 2',
      title: option2.text || 'Skip just one delivery',
      subtitle: '-',
      type: 'option',
    });
    ...
    rows.push({
      ...
      sectionKey: 'option3',
      section: 'Option 3',
      title: option3.text || 'Pause for a while',
      subtitle: '-',
      type: 'option',
    });
    ...
    rows.push({
      ...
      section: 'Note',
      title: '-',
      subtitle: note.text || 'If none of these work for you right now, we understand.',
      type: 'note',
    });
    ...
    rows.push({
      ...
      title: ctas.submitText || 'Continue with cancellation',
      subtitle: ctas.cancelText || 'Keep my subscription',
      type: 'ctas',
    });
  }
  ...
  const columns = [
    {
      ...
      cell: (row: CrmCancelationRow) => (
        <span className='font-medium text-gray-800'>{row.section}</span>
      ),
    },
    {
      id: 'title',
