...
type CrmBenefitsRow = {
  ...
  statList?: Record<string, unknown>[];
  advantageList?: Record<string, unknown>[];
  imagePaths: string[];
};
...
export default function CrmBenefitsPage() {
  ...
  const crmBenefitsRows: CrmBenefitsRow[] = defaultSections.map((def, index) => {
    const sectionData = pageData?.sections?.find((s: PageSection) => s.sectionKey === def.sectionKey);
    ...
    return {
      ...
      section: def.section,
      title: (content.title as string) || def.title,
      subtitle: (content.subtitle as string) || def.subtitle,
      points: (content.points as string[]) || def.points,
      detailList: (content.detailList as Record<string, unknown>[]) || def.detailList,
      statList: (content.statList as Record<string, unknown>[]) || def.statList,
      advantageList: (content.advantageList as Record<string, unknown>[]) || def.advantageList,
      imagePaths: (content.imagePaths as string[]) !== undefined ? (content.imagePaths as string[]) : def.imagePaths,
    };
  });
